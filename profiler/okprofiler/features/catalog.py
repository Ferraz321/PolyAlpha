...
FACTOR_DEFINITIONS = [
    ...
    fd("microstructure_entry_edge", "microstructure entry edge", "high", 0.75, "microstructure", "max(entry_forward_edge, 0) * (ofi_filled + depth_imbalance_filled + price_momentum - spread_filled)", "features.reverse_engineering._add_cross_category_edge_factors", None, ("clob_features", "fills"), ("microstructure_liquidity_timing",), validation_role="diagnostic"),
    ...
    fd("microstructure_pressure_edge", "microstructure pressure edge", "high", 0.75, "microstructure", "max(entry_forward_edge, 0) * microstructure_pressure_score", "features.reverse_engineering._add_cross_category_edge_factors", None, ("clob_features", "fills"), ("microstructure_liquidity_timing",), validation_role="diagnostic"),
    ...
    fd("settlement_window_edge", "settlement window edge", "high", 0.75, "settlement_timing", "is_last_24h * max(entry_forward_edge, 0)", "features.reverse_engineering._add_cross_category_edge_factors", None, ("markets", "fills"), ("settlement_timing",), validation_role="diagnostic"),
    fd("settlement_urgency_edge", "settlement urgency edge", "high", 0.75, "settlement_timing", "is_last_24h * max(entry_forward_edge, 0) / (1 + resolution_lead_time_hours)", "features.reverse_engineering._add_cross_category_edge_factors", None, ("markets", "fills"), ("settlement_timing",), validation_role="diagnostic"),
    fd("same_market_reentry_count", "same-market reentry count", "high", 0.75, "behavior", "count rows over full account + market_id; profile descriptor, not causal", "features.behavior.add_behavior_factors", None, ("fills",), validation_role="diagnostic"),
    fd("buy_ratio", "wallet buy-side ratio", "high", 0.50, "behavior", "mean(side == buy) over full account; profile descriptor, not causal", "features.behavior.add_behavior_factors", None, ("fills",), validation_role="diagnostic"),
    fd("prior_same_market_trade_count", "prior same-market trade count", "high", 0.75, "behavior", "count prior rows over account + market_id before current fill", "features.behavior.add_behavior_factors", None, ("fills",)),
    fd("prior_market_reentry_rate", "prior same-market add rate", "high", 0.75, "behavior", "prior_same_market_trade_count / prior account trade count", "features.behavior.add_behavior_factors", None, ("fills",)),
    fd("prior_repeat_entry_motif_count", "prior repeat entry motif count", "high", 0.75, "motif", "count prior rows over account + market_id + side + entry_hour_utc", "features.behavior.add_behavior_factors", None, ("fills",), ("sector_information_edge", "event_news_information_edge", "cross_sector_scanner")),
    fd("prior_repeat_hour_motif_score", "prior repeat hour motif score", "high", 0.75, "motif", "prior rows over account + entry_hour_utc divided by prior account trade count", "features.behavior.add_behavior_factors", None, ("fills",)),
    fd("prior_buy_ratio", "prior wallet buy-side ratio", "high", 0.50, "behavior", "prior buy fills divided by prior account fills", "features.behavior.add_behavior_factors", None, ("fills",)),
    fd("hour_motif_timing_edge", "hour motif timing edge", "high", 0.75, "motif", "entry_hour_utc * prior_repeat_hour_motif_score", "features.interactions.add_interaction_factors", None, ("fills",), ("event_news_information_edge",)),
    fd("temperature_motif_edge", "temperature motif edge", "high", 0.75, "weather_timing", "temperature_mid_f * prior_repeat_hour_motif_score", "features.interactions.add_interaction_factors", None, ("fills",), ("weather_temperature",)),
    fd("prior_reentry_hour_motif_edge", "prior reentry hour motif edge", "high", 0.75, "motif", "prior_same_market_trade_count * prior_repeat_hour_motif_score", "features.interactions.add_interaction_factors", None, ("fills",), ("event_news_information_edge",)),
    fd("prior_reentry_buy_bias_edge", "prior reentry buy-bias edge", "high", 0.75, "behavior", "prior_same_market_trade_count * prior_buy_ratio", "features.interactions.add_interaction_factors", None, ("fills",)),
    fd("forward_price_move", "next market price move", "high", 0.50, "reverse_engineering", "next market fill price - current fill price after sorting by market_id,timestamp", "features.reverse_engineering._add_forward_move_factors", None, ("fills",), validation_role="target"),
    fd("entry_forward_edge", "directional entry edge", "high", 0.75, "reverse_engineering", "buy: next price - price; sell: price - next price", "features.reverse_engineering._add_forward_move_factors", None, ("fills",), validation_role="target"),
    fd("entry_before_move_secs", "entry before favorable move", "low", 0.50, "reverse_engineering", "seconds to next market fill when entry_forward_edge > 0", "features.reverse_engineering._add_forward_move_factors", None, ("fills",), ("event_news_information_edge",), validation_role="diagnostic"),
    fd("lead_time_evidence", "entry lead-time evidence", "high", 0.75, "reverse_engineering", "entry_forward_edge / (1 + next_lag_hours) when next_lag_secs > 0", "features.reverse_engineering._add_forward_move_factors", None, ("fills",), ("event_news_information_edge",), validation_role="diagnostic"),
    fd("entry_price_advantage", "entry price advantage", "high", 0.75, "reverse_engineering", "for buys, market mean price - fill price", "features.reverse_engineering._add_exit_quality_factors", None, ("fills",), validation_role="diagnostic"),
    fd("exit_quality_proxy", "exit quality proxy", "high", 0.75, "reverse_engineering", "for sells, sell price - wallet average buy price in same market", "features.reverse_engineering._add_exit_quality_factors", None, ("fills",), ("settlement_timing",), validation_role="diagnostic"),
    ...
    fd("sector_entry_edge", "sector entry edge", "high", 0.75, "sector", "mean(entry_forward_edge) over account + sector", "features.reverse_engineering._add_cross_category_edge_factors", None, ("markets", "fills"), ("sector_information_edge",), validation_role="diagnostic"),
    fd("sector_repeat_edge_score", "sector repeat edge score", "high", 0.75, "sector", "sector_concentration * max(entry_forward_edge, 0)", "features.reverse_engineering._add_cross_category_edge_factors", None, ("markets", "fills"), ("sector_information_edge",), validation_role="diagnostic"),
    fd("sector_motif_consistency_edge", "sector motif consistency edge", "high", 0.75, "sector", "sector_concentration * repeat_hour_motif_score * max(entry_forward_edge, 0)", "features.reverse_engineering._add_cross_category_edge_factors", None, ("markets", "fills"), ("sector_information_edge", "cross_sector_scanner"), validation_role="diagnostic"),
    ...
    fd("news_lead_entry_edge", "news lead-entry edge", "high", 0.75, "news", "news_reaction_window * max(entry_forward_edge, 0)", "features.reverse_engineering._add_cross_category_edge_factors", None, ("news", "fills"), ("event_news_information_edge",), validation_role="diagnostic"),
    fd("news_recency_decay_edge", "news recency decay edge", "high", 0.75, "news", "max(entry_forward_edge, 0) / (1 + news_recency_hours)", "features.reverse_engineering._add_cross_category_edge_factors", None, ("news", "fills"), ("event_news_information_edge",), validation_role="diagnostic"),
    fd("repeat_hour_motif_score", "repeat hour motif score", "high", 0.75, "motif", "count(account, entry_hour_utc) / count(account); profile descriptor, not causal", "features.reverse_engineering._add_repeated_motif_factors", None, ("fills",), validation_role="diagnostic"),
    fd("repeat_entry_motif_count", "repeat entry motif count", "high", 0.75, "motif", "count(account, market_id, side, entry_hour_utc); profile descriptor, not causal", "features.reverse_engineering._add_repeated_motif_factors", None, ("fills",), validation_role="diagnostic"),
    fd("repeat_market_add_rate", "repeat market add rate", "high", 0.75, "motif", "same_market_reentry_count / count(account); profile descriptor, not causal", "features.reverse_engineering._add_repeated_motif_factors", None, ("fills",), validation_role="diagnostic"),
    fd("event_motif_recurrence", "event motif recurrence", "high", 0.75, "motif", "repeat_hour_motif_score * repeat_entry_motif_count * max(entry_forward_edge, 0)", "features.reverse_engineering._add_cross_category_edge_factors", None, ("fills",), ("sector_information_edge", "event_news_information_edge", "cross_sector_scanner"), validation_role="diagnostic"),
    ...
    fd("official_station_basis", "official station basis", "low", 0.50, "weather_official", "official_high_temp_f - actual_high_temp_f; realized after event", "features.weather._add_weather_event_context_factors", None, ("weather_events", "weather_observations"), ("weather_temperature",), validation_role="diagnostic"),
    ...
    fd("actual_temp_distance_to_bucket", "actual temp distance to bucket", "low", 0.50, "weather_observation", "distance from actual_high_temp_f to parsed bucket; realized after event", "features.weather_observations.add_weather_observation_factors", None, ("weather_observations",), ("weather_temperature",), validation_role="diagnostic"),
    fd("actual_temp_inside_bucket", "actual temp inside bucket", "high", 0.50, "weather_observation", "1 when actual_high_temp_f is inside parsed bucket; realized after event", "features.weather_observations.add_weather_observation_factors", None, ("weather_observations",), ("weather_temperature",), validation_role="diagnostic"),
    fd("actual_temp_error_to_mid_f", "actual temp error to midpoint", "low", 0.50, "weather_observation", "actual_high_temp_f - temperature_mid_f; realized after event", "features.weather_observations.add_weather_observation_factors", None, ("weather_observations",), ("weather_temperature",), validation_role="diagnostic"),
    ...
    fd("forecast_bias_error_f", "forecast realized bias error", "high", 0.50, "weather_forecast", "official_high_temp_f or actual_high_temp_f minus forecast_temp_f; realized after event", "features.weather_forecasts._add_city_bias_factors", None, ("weather_forecasts",), ("weather_temperature",), validation_role="diagnostic"),
    fd("city_temperature_bias_edge", "city temperature bias edge", "high", 0.75, "weather_forecast", "abs(mean(forecast_bias_error_f) over weather_city); realized after event", "features.weather_forecasts._add_city_bias_factors", None, ("weather_forecasts",), ("weather_temperature",), validation_role="diagnostic"),
    fd("bucket_distance_from_normal", "bucket distance from city normal", "high", 0.75, "weather_forecast", "abs(temperature_mid_f - median(forecast_temp_f) over weather_city); profile descriptor until made as-of", "features.weather_forecasts._add_bucket_normal_factors", None, ("weather_forecasts",), ("weather_temperature",), validation_role="diagnostic"),
]
